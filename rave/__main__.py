...
Uso:
    ...
    python -m rave info --checkpoint runs/rave_v1/checkpoints/rave_final.pt
'''
...
def _add_train_args(sp: argparse.ArgumentParser) -> None:
    ...
    sp.add_argument('--n-steps', type=int, default=100_000, help='total de pasos de entrenamiento a ejecutar en esta corrida (no incluye los del checkpoint reanudado)')
    ...
    # reanudar/finetune
    sp.add_argument('--resume-from', type=pathlib.Path, default=None, help='ruta a un checkpoint desde el cual reanudar; sin este argumento el entrenamiento arranca desde cero')
    sp.add_argument('--resume-mode', choices=['continue', 'finetune'], default='continue', help='continue: restaura pesos, optimizer y step del checkpoint (retomar el mismo entrenamiento). finetune: solo restaura pesos, optimizer y step arrancan desde cero (transfer learning a otro audio)')
